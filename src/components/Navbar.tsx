    ...
    <nav className="sticky top-0 z-50 border-b border-border bg-background/80 backdrop-blur-md">
        ...
        <div className="hidden items-center gap-6 md:flex">
          ...
          {user ? (
            <div className="flex items-center gap-3">
              ...
              <button onClick={logout} className="text-muted-foreground hover:text-foreground transition-colors"><LogOut size={18} /></button>
            </div>
          ) : (
            <Link to="/login" className="flex items-center gap-1 text-sm font-medium text-muted-foreground hover:text-foreground transition-colors">
              <User size={18} /> Login
            </Link>
          )}
        </div>
      ...
      {open && (
        <div className="border-t border-border bg-background p-4 md:hidden flex flex-col gap-3">
          ...
          {user && <Link to="/orders" onClick={() => setOpen(false)} className="text-sm font-medium">Orders</Link>}
          <Link to="/track" onClick={() => setOpen(false)} className="text-sm font-medium">Track</Link>
          <Link to="/cart" onClick={() => setOpen(false)} className="text-sm font-medium flex items-center gap-2">
            Cart {cartCount > 0 && <span className="rounded-full bg-primary px-2 py-0.5 text-[10px] text-primary-foreground">{cartCount}</span>}
          </Link>
          {user ? (
            <button onClick={() => { logout(); setOpen(false); }} className="text-sm font-medium text-left">Logout</button>
          ) : (
